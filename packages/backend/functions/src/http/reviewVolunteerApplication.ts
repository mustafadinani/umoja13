...
import { volunteerApplicationDecisionEmail } from "../util/emailTemplates.js";
 ...
 */
export const reviewVolunteerApplication = onCall<ReviewVolunteerApplicationRequest>(
  ...
  async (request) => {
    ...
    if (decision === "approve") {
      ...
      await sendDecisionEmail(application, true);
      return { status: "approved" };
    }

    await ref.set({ status: "rejected", reviewedBy: uid, reviewedAt: now }, { merge: true });
    await sendDecisionEmail(application, false);
    return { status: "rejected" };
  }
);

async function sendDecisionEmail(application: VolunteerApplication, approved: boolean): Promise<void> {
  if (!application.email) return;
  try {
    const { subject, html } = volunteerApplicationDecisionEmail(application.name, approved);
    await sendEmail(application.email, subject, html);
  } catch (err) {
    console.error("reviewVolunteerApplication: failed to send decision email:", err);
  }
}
